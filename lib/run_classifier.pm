...
sub get_antigens_from_PDB_chain_list {
    my $file = shift;
    open(my $IN, "<", $file) or die "Cannot open list of PDB chains $file, $!";
    my @antigens;
    while (my $line = <$IN>) {
        chomp $line;
        my ($pdb_code, @chainIDs) = split(/\s+/, $line);
        push(@antigens, get_chains_from_PDB($pdb_code, 'pdb_code', @chainIDs));
    }
    return @antigens;
}

sub get_antigens_from_PDB_file_list {
    my $file = shift;
    open(my $IN, "<", $file) or die "Cannot open list of PDB files $file, $!";
    my @antigens;
    while (my $line = <$IN>) {
        chomp $line;
        my ($file_name, @chainIDs) = split(/\s+/, $line);
        push(@antigens, get_chains_from_PDB($file_name, 'pdb_file', @chainIDs));
    }
    return @antigens;
}

sub get_chains_from_PDB {
    my ($input, $input_type, @chainIDs) = @_;
    my @chains = pdb->new($input_type      => $input,
                          solvent_cleanup  => 1,
                          het_atom_cleanup => 1)->create_chains(@chainIDs);
    return @chains;
}
